  ...
  return (
    ...
    >
      ...
      <svg
        ...
        fill="none"
        aria-hidden="true"
        /* ⚠️ display:"inline" IS LOAD-BEARING IN THIS REPO, and is the one line that differs from
           logo/ThemisIQLogo.tsx as delivered. Tailwind 4's Preflight sets
           `img, svg, video, … { display: block }` (node_modules/tailwindcss/preflight.css:209),
           so without this the Q becomes a block, takes its own line, and the lockup renders as
           "ThemisI" above a stray mark — on every surface, at every size. white-space:nowrap on
           the parent does not save it: a block-level child breaks the line regardless.
           The -0.163em baseline offset below is calibrated for an INLINE replaced element, so
           inline is the faithful restore, not inline-block. */
        style={{ display: "inline", verticalAlign: "-0.163em", overflow: "visible" }}
      >
