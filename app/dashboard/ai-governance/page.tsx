...
// by EU product-safety law (Article 6(1)). It therefore cannot pick a single date, and must not guess —
// so every per-system surface prints both and names the distinction. `useEffect` went with the
// countdown this replaces: it computed days-to-2026-08-02 and clamped at zero, so a passed date read
// as "0 days" urgency rather than as broken.
const HIGH_RISK_BOTH_DATES =
  `${AI_ACT_HIGH_RISK_STANDALONE} (stand-alone) · ${AI_ACT_HIGH_RISK_EMBEDDED} (in a regulated product)`
...
export default function AIGovernanceDashboard() {
  ...
  const generateExport = () => {
    const rows = [
      ...
      ...inventory.systems.map(s => [s.name, s.purpose, RISK_CONFIG[s.risk_level].label, s.annex_category, s.requirements.join(' | '), s.risk_level === 'prohibited' ? 'IMMEDIATE' : s.risk_level === 'high_risk' ? HIGH_RISK_BOTH_DATES : 'Ongoing']),
      [''],
            ...
            {inventory.systems[activeSystem].risk_level !== 'unclassified' && (
              <div style={{ background: RISK_CONFIG[inventory.systems[activeSystem].risk_level].bg, border: `1px solid ${RISK_CONFIG[inventory.systems[activeSystem].risk_level].border}`, borderRadius: 10, padding: '1rem' }}>
                ...
                {inventory.systems[activeSystem].risk_level === 'high_risk' && (
                  <div style={{ fontSize: 11, color: '#B91C1C', marginTop: 6, fontWeight: 600 }}>Conformity assessment required from {HIGH_RISK_BOTH_DATES}</div>
                )}
              </div>
            )}
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', background: '#f8f7f5', minHeight: '100vh' }}>
      ...
      {/* ⚠️ INK ON THE FILL, NOT WHITE. Inverted 26 Sep 2026 with the colourway. White on this fill
          measures 2.85:1 — below AA for 12-13px text — because the 2026 module values are light. Measured
          with --color-ink instead: 6.15:1. Only Climate Risk's #004AAD still takes white, and it has no bar.
      ...
      <div style={{ background: 'var(--color-module-ai)', padding: '8px 2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-ink)' }}>EU AI Act high-risk obligations from {HIGH_RISK_BOTH_DATES}. Start your inventory now.</span>
      </div>
