...
const SECTOR_RISKS: Record<string, { risk: string; severity: 'critical' | 'high' | 'medium'; framework: string; detail: string }[]> = {
  'Energy & Utilities': [
    { risk: 'High Scope 1 emissions exposure', severity: 'critical', framework: 'SB 253 / CSRD', detail: 'Energy companies typically carry 60-80% of portfolio Scope 1 emissions. SB 253 requires full consolidation into buyer\'s inventory.' },
...
export default function DealsDashboard() {
  ...
  const generateExport = () => {
    const rows = [
      ...
      ...risks.map(r => [r.risk, r.severity.toUpperCase(), r.framework, r.detail]),
      [],
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', background: '#f8f7f5', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#0C447C', padding: '8px 2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 500, color: '#fff' }}>IFRS S2 effective for FY2024 · SB 253 Scope 3 deadline 2027 · CSRD applies to large EU companies now. ESG is a material deal risk.</span>
      </div>
          ...
          {step < 4 && (
            <div style={{ position: 'sticky', top: 80 }}>
              ...
              <div style={{ marginTop: 10, background: '#E6F1FB', border: '0.5px solid rgba(12,68,124,0.2)', borderRadius: 10, padding: '0.75rem' }}>
                <div style={{ fontSize: 11, color: '#0C447C', lineHeight: 1.6 }}><strong>IFRS S2 · TCFD · SB 253</strong><br />ESG is now a material deal risk</div>
              </div>
            </div>
          )}
        </div>
