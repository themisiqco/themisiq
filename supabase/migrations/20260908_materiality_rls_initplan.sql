-- ══════════════════════════════════════════════════════════════════════════════════════════════
-- ⚠️  RUN ON 21 SEP 2026, IN FULL, AGAINST PRODUCTION.  BATCH 2 OF 6.
-- ══════════════════════════════════════════════════════════════════════════════════════════════
--
...
--
-- ── WHAT THIS ONE DID ──────────────────────────────────────────────────────────────────────────
-- 16 materiality_* policies were dropped and recreated with (select auth.uid()), read from
-- pg_policies at run time. Policies still carrying a bare auth.uid() immediately afterwards:
-- 52 in public, 6 in storage.
--
-- ── RUNNING IT AGAIN ────────────────────────────────────────────────────────────────────────────
-- It aborts before dropping anything. Its capture matches on auth.uid() without excluding the
-- wrapped form, so a second run would still find all 16; what stops it is the separate pre-flight
-- guard that raises when any captured predicate already contains a wrapped auth.uid().
--
