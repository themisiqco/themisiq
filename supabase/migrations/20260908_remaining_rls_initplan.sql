...
--
-- Scope note: this file selects by EXCLUSION (schemaname = 'public' and tablename <> 'audit_log'),
-- not from an enumerated table list, so it also sweeps up policies the repo cannot see. That is
-- the right choice given the divergence above — but it means the set it touches CANNOT be
-- predicted from this repo. Read the NOTICE output when it runs; that is the only place the
-- actual list appears.
--
-- 20260908_remaining_rls_initplan.sql
--
-- BATCH 5a of 5 — Remaining (public). Wrap auth.uid() in a scalar subselect.
--
...
--
-- ⚠️ SCOPE IS AN EXCLUSION, NOT AN INCLUSION LIST, AND THAT IS DELIBERATE.
-- This batch is "everything still bare in the public schema", so it also sweeps up any policy the
-- repo cannot see — the twelfth CBAM policy, the six materiality policies the advisor counts and
-- supabase/migrations does not, and anything else created in the dashboard. An inclusion list
-- would have inherited the repo's blind spot.
--
-- ⚠️ audit_log IS EXCLUDED, PENDING A DECISION THAT HAS NOT BEEN MADE.
-- `audit_log.audit_select_own` references auth.uid(). It was raised in batch 3 and left out
-- because the audit trail's append-only behaviour is an Annex II commitment in the DPA. Append-only
-- comes from the ABSENCE of any UPDATE or DELETE policy, so rewriting a FOR SELECT policy could not
-- create a write path — but that is an argument, and the commitment deserves a decision. It will
-- therefore still show as an auth_rls_initplan finding after this batch. That is expected, not a
-- miss. To include it, delete the `tablename <> 'audit_log'` line from both scopes below.
...
begin
  ...
  if v_n = 0 then
    raise exception 'Pre-flight: nothing in scope for batch 5a of 5. Already run, or the scope is wrong.';
  end if;
  raise notice 'Batch 5a of 5 (Remaining (public)): % policies in scope —', v_n;
...
begin
  ...
  raise notice 'Batch 5a of 5 (Remaining (public)): % policies re-emitted and verified.', v_total;
end $$;
