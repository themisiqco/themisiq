...
--
-- Scope note: this file touches the `storage` schema, which Supabase manages. It is separated from
-- batch 5a for that reason — a failure here should not roll back the public-schema work, and the
-- decision to alter a Supabase-managed schema should be taken on its own rather than carried along
-- inside a larger batch.
--
-- 20260908_storage_rls_initplan.sql
--
-- BATCH 5b of 5 — storage.objects. Wrap auth.uid() in a scalar subselect.
--
...
--
-- ⚠️ SEPARATE FILE BECAUSE IT TOUCHES A SCHEMA SUPABASE MANAGES.
-- These six policies are user-created bucket rules on storage.objects — three for the GHG
-- 'source-documents' bucket, three for 'cbam-source-documents' — but they live in storage, not
-- public, and re-emitting anything there can collide with a platform update. Kept as its own file
-- so it can be run, skipped or rolled back independently of 5a.
--
-- Note the predicates cast: `(auth.uid())::text = (storage.foldername(name))[1]`. After the
-- rewrite that reads `((select auth.uid()))::text`, which is the same expression with the same
-- cast — the verification block confirms it rather than trusting it.
...
begin
  ...
  if v_n = 0 then
    raise exception 'Pre-flight: nothing in scope for batch 5b of 5. Already run, or the scope is wrong.';
  end if;
  raise notice 'Batch 5b of 5 (storage.objects): % policies in scope —', v_n;
...
begin
  ...
  raise notice 'Batch 5b of 5 (storage.objects): % policies re-emitted and verified.', v_total;
end $$;
