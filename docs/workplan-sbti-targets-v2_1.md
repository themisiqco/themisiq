...
- `renewalTightening(prevTarget, performance) → reductionPct` — the ratchet for V2.0 renewal targets. **⛔ BLOCKED — not built. External dependency:** the V2.0 ratchet *principle* exists (underperformers get tighter renewal targets at cycle end, ≥90%-achievement gate), but the exact tightening *formula/multiplier* is defined in the **SBTi Renewal Policy, expected Q4 2026** — after today (2026-06-29). V2.0 is not usable for target-setting until Q1 2027 regardless. Building an invented multiplier now would violate "accuracy forms trust." **Unblock when:** the SBTi Renewal Policy publishes the tightening rule; implement then as a params-driven function mirroring the rest of the engine. The other 7 engine functions (`categorize`, `validateTargetConfig`, `acaSuggestedReductionPct`, `computeTrajectory`, `progressStatus`, `trajectoryPointForYear`, `requiredScope3Categories`, `cycleState`) are complete and committed; `cycleState` was deliberately built last because it needs nothing from the unpublished policy.
