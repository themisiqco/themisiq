...
- **Pin — authenticated grants not yet captured:** `grant select on cbam_production_processes, cbam_installations, cbam_source_streams, cbam_precursor_inputs to authenticated; grant select, insert on cbam_see_records to authenticated;` — applied live, needs a migration (same idempotent pattern as the reference grants file).
