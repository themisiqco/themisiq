...
|---|---|---|
| `cbam_cn_map.cn_prefix` | digits only, **no spaces** | **58 rows** = 54× 4-digit, 3× 5-digit, 1× 8-digit (44 steel + 14 aluminium; re-derived 29 Jul 2026 — the former "40× 4-digit" was the steel-only count, written before the aluminium seed) |
| `cbam_default_values.cn_code` | spaced, mixed width | **8,052 rows over 224 distinct codes** = 27× 4-digit, 63× 6-digit, 134× 8-digit (verified against production 29 Jul 2026: 200 steel `'other'` + 6,224 steel ISO + 24 aluminium `'other'` + 1,604 aluminium ISO). **The migrations declare 8,251 tuples** — 199 more than exist as rows: `20260727_cbam_steel_percountry_defaults.sql` re-declares 199 of the 16 Jul `'other'` rows and discards them via `on conflict (cn_code, country) do nothing`, by design and byte-identical to the originals, so no value is overridden. 199 not 200 because `2601 12 00` is chapter-26 and outside that file's 72/73 scope. The former "19× 4-digit, 62× 6-digit, 119× 8-digit" was the steel-only distribution, where one row per code made rows and codes identical; the per-country seeds broke that, so length distribution is stated over the CODE SET and the row total carried separately |
| `cbam_benchmarks` (IR 2025/2620) | spaced, **always 8-digit** | **2,389 rows over 536 distinct codes**, all 8-digit (2,210 steel + 179 aluminium; re-derived 29 Jul 2026 — the former "570 rows" matches no count in either seed and appears to have been an early estimate. Nearest actual figure is 560 Column-A rows in the steel seed) |
...
**10.9 Tooling hazards — THREE ways a naive read of these migrations returns a wrong count.** All three were hit in practice. Any tooling reading these files must handle all three, and should assert expected counts rather than trusting its own parse.

*(a) A semicolon inside a SQL COMMENT terminates the statement.* The `cbam_cn_map` INSERT contains one (`-- 7205 dual-listed; pig-iron-granule…`). A naive `;`-split parser silently captures 11 of 44 rows. Detected only by asserting the documented row count (44).

*(b) A semicolon inside a STRING LITERAL does the same — comment-stripping alone is NOT sufficient.* The aluminium `cbam_production_routes` seed carries `'Hall-Héroult primary smelting; direct incl. PFC anode-effect events'` in a `boundary_note`. A comment-aware but quote-blind parser silently returns 1 aluminium route and 1 aluminium category instead of 4 and 2. A parser must respect quoted strings, not merely comments. Added 29 Jul 2026.

*(c) A TUPLE COUNT IS NOT A ROW CONTRIBUTION where a seed ends in `on conflict … do nothing`.* `20260727_cbam_steel_percountry_defaults.sql` DECLARES 6,423 tuples and CONTRIBUTES 6,224 rows: its first 199 re-declare `'other'` rows already seeded on 16 Jul and are discarded on conflict, by design and byte-identical to the originals. Parsing migrations therefore yields 8,251 for `cbam_default_values` where production holds 8,052. **Any row-count claim derived by parsing migrations must state which of the two it is** — and a figure that matters should be verified against the database, not the files. Verified 29 Jul 2026. Added 29 Jul 2026.
