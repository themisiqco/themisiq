...
* **Terms §5 references "Part C (Checkout Consent)", which does not exist as a page in this repo.**
  The consent wording a customer actually sees is the three checkboxes in
  `app/components/ConsentForm.tsx`, reproduced at the end of this file. This is more than an
  extraction detail — see the next section.

The Terms and Privacy pages carry an intro callout above their first section; both are included.

## ⚠️ Part C is unresolved, and this snapshot may therefore be incomplete

**State this before relying on the file.** Consent version `2026-06-v2-final` is described in the
code as covering a three-document bundle, and only two of those three are in this repo.

What is established:

* **Terms §5 cites it by name** — "See Part C (Checkout Consent)" — as though it were a document a
  reader could turn to.
* **`ConsentForm.tsx` names the same bundle twice**, at `:13` ("Counsel-final consent wording version
  (Terms / Refund Policy / Consent Part C)") and at `:69` ("Terms/Refund/Consent Part C;
  counsel-final"), with the instruction "do NOT change without legal sign-off". Two independent
  comments treat the bundle as a real, counsel-produced set.
* **No Part C exists in this repo in any form.** Searched on 2026-09-09 at commit `0a573d9`:
  the phrase "Checkout Consent" appears exactly once, in Terms §5 itself; no file, route or
  directory matches `part-c`, `consent` or `legal`; there is no `/consent` URL; and
  `git log --all --diff-filter=D` shows no deleted policy document, ever. Nothing was removed —
  it was never committed.
* **No Part A or Part B is named either.** The letters are never assigned to the Terms or the
  Refund Policy anywhere in the repo, so even the mapping is inferred rather than recorded.
  `CLAUDE.md:262` names a `docs/terms-part-a.md` that has never been committed and is not on disk.

**What this means for this file.** The three consent checkboxes reproduced in section 4 below may
BE Part C — the whole of it — or they may be a summary of a separate counsel-drafted instrument
held outside this repository. The code cannot distinguish the two. **Until that is confirmed
against what counsel actually delivered in June 2026, this snapshot may be an incomplete record of
what `2026-06-v2-final` covers**, and a customer's consent row may point to wording that is only
partly reproduced here.

Resolving it is one question to counsel or to whoever holds the June 2026 delivery. If Part C turns
out to be a separate document, add it to this file and note the correction rather than replacing
the file — a snapshot that changed silently is worth less than one that records what it once
lacked.
